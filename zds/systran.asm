...
;;
;; Revisions:
;; 20140912	- Modified for CP/M3 and hd support
;;		  adapted to new modular SYSBIOS
;;
...
;
VERSMA	EQU	2			; VERSION MAJOR,MINOR
VERSMI	EQU	0
; BDOS	EQU	$0005			; BDOS FUNCTION CALL ENTRY PT
;
...
; 	LD	IY,FLSECS		; IY point to current CHS info
	LD	IY,SSIZ5 - 2
; 	LD	C,19H
; 	CALL	BDOS			; get current disk
	...
	CALL	PSTRING
SAGAIN:	LD	DE,MSOUDK		; ASK FOR SOURCE
	CALL	PSTRING
	CALL	ZCI			; GET IT
...
GOTSRC:	LD	DE,MDESDK		; ASK FOR DESTINATION
	...
	;
GOTDST:
	LD	DE,MINSED		; PROMPT USER FOR SOURCE IN DRIVE
	CALL	PSTRING
	LD	DE,MSOURC
	CALL	PSTRING
	LD	DE,MPRESS
...
RNSEC:	LD	BC,(COFF)		; TRANSFER PARAMS TO BIOS
	...
	ISHD?
	JR	NZ,RSSEC		; read HD sector
	ISVHD?
	JR	NZ,RSVRT		; read virtual HD sector
	; ------------------------------------------------
	LD	DE,TRANS
	CALL	LSECTRA
	LD	B,H
...
BRDVRT:	CALL	BBRDVDSK		; call par. read
	JR	RGO
RDIDE:	CALL	BBHDRD			; call ide read
