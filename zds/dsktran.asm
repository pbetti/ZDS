...
; 	jr	asksize
	...
	call	dprmset
	call	inline
	defb	cr,lf,"Is a NEDOS disk ? (y/n): ",0
	call	gchr			;
	call	usrexit
	cp	'Y'			; is 'y' ?
	jr	nz,nodos		; no
	call	inline
	defb	"es, NEDOS format.",cr,lf,lf,0
	xor	a
	ld	(nedos),a
	jr	askconf
nodos:
...
askdrives:
	...
	call	inline
	defb	cr,lf,"Press any key to start, when ready...",cr,lf,0
	call	bbconin
	call	usrexit
	jr	recover2		; go on
...
fretr1:
	ld	a,(fsecbuf)
	ld	c,a			; load sec #
	ld	a,(nedos)		; are we in nedos mode?
	or	a
	jr	nz,stdfsk		; no
	dec	c			; yes sec # in zero based
...
frwwro:
	...
	or	a
	jr	nz,stdfwr		; no
	ld	a,(ftrkbuf)		; yes, n track 17?
	cp	17
	jr	nz,stdfwr		; no
	ld	a,$a9			; write cmd for FA DAM
	jr	sndwrc
stdfwr:	ld	a,fdcwritc		; write command
sndwrc:	out	(fdccmdstatr),a		; exec. command
	call	fdcdly
	jr	fwrdy
frwbsy:
	rrca				; busy bit to carry flag
