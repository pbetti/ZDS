...
;;		full........
;; 20080216	v1.4...
;;		disabled read-after-write, minor optimizations, changed
;;		disk format to 256 directory entries
;; 20140805	Removed interrupt handling. It is in SYSBIOS now.
;;		Modified for banked monitor
;;----------------------------------------------------------------------------
;
...
;
CBREV	EQU	15		; CBIOS version number
	;
	; load common symbols
	;
include common.asm
...
; 	JP      MREADER		; READER   - reader character out
	JP      MNODEV		; LIST     - list character out
	JP      MNODEV		; PUNCH    - punch character out
	JP      MNODEV		; READER   - reader character out
...
; 	JP      MLISTST		; LISTST   - return list status
	JP      MNODEV		; LISTST   - return list status
...
SIGNON:	DEFB	CR,LF
	...
	DEFB	'(c) 2006-14 P. Betti <pbetti@lpconsul.net>'
	DEFB	CR,LF+$80
BFAILMSG:
...
ENDIF
	PUSH	BC
	LD	BC,COLDEND-COLDBEG+1	; copy cmd string
	LDIR
	POP	BC
	JP	CCP			; go to CCP
	;
...
; 	DI				; disable interrupts
	CALL	BBFDRVSEL		; drive activation
 	CALL	BBFHOME			; send cmd.
...
MNODEV:
	...
	;; CONSTR print a string using BBCONOUT
CONSTR:
	PUSH	BC
CSLP0:	LD	C,(HL)
	LD	B,C
