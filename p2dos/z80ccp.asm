...
;
; Modified on 20051113 to restore .VAR syntax to normal EQU equates
;
...
LF	EQU	$0A
TAB	EQU	$09
FF	EQU	$0C		;<==== CHANGE THIS BYTE IF TERMINAL
ESC	EQU	$1B		;      NEEDS A DIFFERENT CHARACTER
				;      TO CLEAR THE SCREEN


CCBOOT	EQU	$0000		; CP/M WARM BOOT ADDRESS
UDFLAG	EQU	$0004		; USER NUMBER IS IN HIGH NYBBLE, DISK IN LOW
BDOSE	EQU	$0005		; BDOS FUNCTION CALL ENTRY PT
TBUFF	EQU	$0080		; DEFAULT DISK I/O BUFFER
TFCB	EQU	$005C		; DEFAULT FCB BUFFER
TPA	EQU	$0100		; BASE OF TPA
...
;
CREAD:
	LD	C,$14	; FALL THRU TO GOBDOS
;
GOBDOS:
	CALL	BDOSBB	; PRESERVE B
	OR	A
	RET
;
CWRITE:
...
;
GETUSR:
	LD	E,$FF		;GET CURRENT USER NUMBER
SETUSR:
	LD	C,$20		;SET USER NUMBER TO VALUE IN E (GET IF E=FFH)
	JP	BDOSE
...
UCASE:
	CP	$61		; LOWER-CASE A
	RET	C
	CP	$7B		; GREATER THAN LOWER-CASE Z?
	RET	NC
	AND	$5F		; CAPITALIZE
	RET
...
RB1:
	...
	LD	A,'>'		; PRINT PROMPT
	CALL	CCONOUT
	LD	C,$0A		; READ COMMAND LINE FROM USER
	LD	DE,MBUFF
	CALL	BDOSE
...
BREAK:
	...
	CALL	BDOSBB
	POP	DE
	AND	$7F		; MASK MSB AND SET ZERO FLAG
	RET
...
ERR2:
	...
	CALL	CCONOUT		; PRINT COMMAND CHAR
	POP	HL		; GET PTR
	INC	HL		; PT TO NEXT
	JR	ERR2		; CONTINUE
ERR1:
...
CCPE:
	...
	POP	BC
	LD	A,C		; C=USER/DISK NUMBER (SEE LOC 4)
	AND	$F		; EXTRACT DEFAULT DISK DRIVE
	LD	(TDRIVE),A	; SET IT
	CALL	CLOGIN		; LOG IN DEFAULT DISK
...
CNUM1:
	...
	AND	$E0		; CHECK FOR RANGE ERROR (>255)
	JP	NZ,ERROR
...
;*
DIR:
	LD	A,$80		; SET SYSTEM BIT EXAMINATION
	PUSH	AF
...
DIR3:
	...
	INC	E
	PUSH	DE		; SAVE IT
	AND	3		; OUTPUT <CRLF> IF 4 ENTRIES PRINTED IN LINE
	PUSH	AF
	JR	NZ,DIR4
	CALL	CRLF		; NEW LINE
	JR	DIR5
...
ERA1:
	...
	LD	A,$80		; SKIP SYSTEM FILES (EXAMINE SYSTEM BIT)
...
TYPE0:
	...
	CALL	SLOGIN		; LOG IN SELECTED DISK IF ANY
	CALL	COPENF		; OPEN SELECTED FILE
...
COMA:
	...
	CALL	COPENF		; OPEN COMMAND.COM FILE
	JR	NZ,COMA1	; ERROR?
...
COM2:
	...
	LD	DE,CENTRY-128	; ARE WE GOING TO WRITE OVER CCP?
...
COM3:
	...
	LD	(FCBDN),A
	LD	A,$10		; OFFSET FOR 2ND FILE SPEC
