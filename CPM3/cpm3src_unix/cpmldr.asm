...
high$fxs:
	sbi 100
	 jc lret$eq$ff 		; Skip if function < 100
...
selectdisk:
	...
	mov a,h
	 ora l
	 rz 	; Return with C flag reset if select error
...
seek2:
	xthl
	 push h 			; hl,s0 = curtrk, s1 = low(arecord)
	lhld sectpt
	 call bde$e$bde$m$hl 	; currec = currec - sectpt
	pop h
	 push d
...
get$de1:
	dcx h
	 dcr b			; Decr dskmap position
	cmp m
	 jnz get$de2 		; fcb(dskmap(b)) ~= 0
	dcr c
	 jnz get$de1
...
get$de3:
	push b
	 push h 			; Save dskmap position & count
	mov l,a
	 mvi h,0 		; hl = non-zero blk idx
...
open$copy:
	call getexta 
	 mov a,m 
	 push a	; save extent to check for extent
...
select:
	...
	mov e,a
	 push d 		; Send to seldsk, save for test below
	call selectdisk
