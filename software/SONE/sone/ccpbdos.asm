...
ENTRYP	EQU	5		;entry point for the cp/m bdos.
...
VERIFY1:LD	A,(DE)
	CP	(HL)
	JP	NZ,SYSHALT	;jump to halt routine.
	INC	DE
	INC	HL
	DEC	B
	JP	NZ,VERIFY1
...
DIRECT3:JP	Z,DIRECT9	;terminate if no more names.
	...
	CALL	PRINTB
	JP	DIRECT5
DIRECT4:CALL	PSPACE		;add seperator between file names.
	LD	A,':'
	CALL	PRINTB
DIRECT5:CALL	PSPACE
...
DRECT65:CALL	PRINTB
	...
	JP	NZ,DIRECT6
	CALL	PSPACE		;yes, add seperating space.
	JP	DIRECT6
...
;
FUNCTNS:DEFW	WBOOT,GETCON,OUTCON,GETRDR,PUNCH,LISTD,DIRCIO,GETIOB
