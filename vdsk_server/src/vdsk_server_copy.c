...
int main( int argc, char *argv[] )       /*{{{*/
{
	...
	int ASize = 0;
	int BSize = 0;
	int count = 0, rsec = 0, rtrk = 0, wsec = 0, wtrk = 0;
	FILE * dump;
...
//	io_sector = ( byte * ) malloc( drive.secLength );
	...
	c = 0;
	while (c < count) {
		printf("reading trk/sec %d/%d, writing trk/sec %d/%d, count=%d\n", rtrk, rsec, wtrk, wsec, c);
		...
		if (dump) {
			fwrite(io_sector, driveB.dev.geom.dg_secsize, 1, dump);
		}
		++wsec;
